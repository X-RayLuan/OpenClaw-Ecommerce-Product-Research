...
> **默认搜索栈 = Exa（§3）+ Tavily（§2）**，仅靠这两个 key 即可跑通全流程。
> §1 的 Pangolin MCP 是**可选增强**：配了用它拿结构化 Amazon 数据 / Google Trends 指数 / 官方 IP 库；
> 没配就跳过 §1，对标发现用 Exa 锁 `amazon.com`、口碑用 Tavily、IP 用 Exa/Tavily 锁 uspto.gov 平替。

---

## 1. 〔可选〕Amazon / Google Trends / Google AI Overview / WIPO / PACER（MCP）

> 未设置 `PANGOLIN_MCP_URL` 时**跳过本节**，用 §2 Tavily + §3 Exa 平替。
...
```bash
# 步骤1 默认对标发现：锁 amazon.com 拿对标 PDP/listing（无 Pangolin 时的主路径）
curl -sS --max-time 60 -X POST 'https://api.exa.ai/search' \
  -H "x-api-key: $EXA_API_KEY" -H 'Content-Type: application/json' \
  -d '{"query":"<节日/利基 + 品类，如：easter door cover banner>","numResults":12,"type":"auto",
       "includeDomains":["amazon.com"],
       "contents":{"text":{"maxCharacters":1500}}}'
# 从 results[].url 里形如 /dp/<ASIN> 提取对标 ASIN；正文里取价/标题/卖点。

# 货源/工厂/独立站竞品（可选 includeDomains 锁定平台；contents.text 直接回正文）
curl -sS --max-time 60 -X POST 'https://api.exa.ai/search' \
  -H "x-api-key: $EXA_API_KEY" -H 'Content-Type: application/json' \
...
# 步骤3 热度定性代理（无 Google Trends 时）：看内容量/上新密度/讨论度
curl -sS --max-time 60 -X POST 'https://api.exa.ai/search' \
  -H "x-api-key: $EXA_API_KEY" -H 'Content-Type: application/json' \
  -d '{"query":"best selling <holiday> <element> decor 2026","numResults":10,"type":"auto","contents":{"text":{"maxCharacters":600}}}'
